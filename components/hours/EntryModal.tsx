...
import { Loader2, Sparkles, Trash2, Copy, ArrowRight, Lock } from "lucide-react";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function EntryModal({
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const body = { projectId, date: state.date, hours, rawNotes: rawNotes || null, aiDescription: aiDescription || null, tagIds };
      const res = existing
        ? await fetch(`/api/entries/${existing.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {datePrompt ? (
            ...
            <div className="space-y-5">
              ...
              {/* Copy / Move / Delete actions for existing entries */}
              {existing && !readonly && (
